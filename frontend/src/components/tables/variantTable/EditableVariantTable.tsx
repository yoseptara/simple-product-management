...
import { EditableCell } from "./EditableVariantCell";
...
import { UPDATE_PATH, VARIANTS_PATH } from "@src/components/router/AppRouter";
import { Variant, VariantList, getVariants } from "@src/api/variants.api";

interface EditableVariantTableProps {
  productId: number; // or string, depending on your use case
}
...
export const EditableVariantTable: React.FC<EditableVariantTableProps> = ({
  productId,
}) => {
  ...
  const edit = (variantId: number) => {
    navigate(`/${productId}${VARIANTS_PATH}/${variantId}${UPDATE_PATH}`);
  };
  // const edit = (record: Partial<Variant> & { key: React.Key }) => {
  ...
  // const handleViewDetail = (variantId: number) => {
  //   navigate(DASHBOARD_PATH + variantId + "/" + variantId);
  // };
